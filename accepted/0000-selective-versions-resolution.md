...
A `resolutions` map compatible with the one used by [Yarn](https://yarnpkg.io) that supports [Selective Versions Resolution](https://github.com/yarnpkg/rfcs/blob/master/implemented/0000-selective-versions-resolutions.md) would be capable of substituting package versions to use.

## Motivation

There are occasions where one wishes
...
If we leverage the `resolutions` or `dependencies` map of each package while traversing the graph breadth-first outward from `my-app@1.0.0`, we will arrive at the following resolved versions, which is basically a flattened version of the tree, preferring shallow nodes:
...
Support a `resolutions` field of the `package.json` file to declare/override version resolution for packages at arbitrary or specific location in the package tree.

# Motivation

Sometimes specific versionsThe motivation was initially discussed in
